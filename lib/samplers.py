...
class DDIM(nn.Module):
    ...
    def sample(self, num_examples=64, num_steps=200):
        ...
        for s, t in zip(ss, ts):
            ...
            d_t = (x_t - alpha(t) * x0_hat) / sigma(t)
            x_s = alpha(s) * x0_hat + sigma(s) * d_t
            x_t = x_s
        return x_t
...
class Heun(nn.Module):
    ...
    def sample(self, num_examples=64, num_steps=200):
        k = num_examples
        alpha = self.schedule.alpha
        beta = self.schedule.beta
        sigma = self.schedule.sigma
        sigma_dot = self.schedule.sigma_dot
        ...
        def dx_dt(x_t, t):
            return -0.5 * beta(t) * x_t + 0.5 * beta(t) * epsilon(x_t, t) / sigma(t)
